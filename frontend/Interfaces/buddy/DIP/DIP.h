...
inline void Corr2DNChannels(cv::Mat &input, MemRef<float, 2> *kernel,
                            cv::Mat &output, unsigned int centerX,
                            unsigned int centerY, BOUNDARY_OPTION option,
                            float constantValue = 0) {
  ...
  for (unsigned i = 0; i < numChannels; ++i) {
    ...
    dip::Corr2D(&inputChannel, kernel, &outputMemRef, centerX, centerY, option,
                constantValue);

    cv::Mat tempMat(input.rows, input.cols, CV_32FC1);
    std::memcpy(tempMat.data, outputMemRef.getData(),
                input.rows * input.cols * sizeof(float));

    outputChannels.push_back(tempMat);
  }
